...
public class OutboundSectorStep implements WizardStep {
    ...
    private boolean saveSectors(){
        ...
        Object obj = UI.getCurrent().getSession().getAttribute("obSectorMap");
        ...
        if(noOfSectors < sectorMap.size()){
            ...
            UI.getCurrent().getSession().setAttribute("obSectorMap",newMap);
        }
        else {
            UI.getCurrent().getSession().setAttribute("obSectorMap", sectorMap);
        }
        return true;
    }
    ...
    private void fillFieldsIfAddedAlready(){
        ...
        if(obj != null){
            ...
            for(Map.Entry<Integer,Sector> map : item.entrySet()){
                ...
                if(sector.getSectorType() != null && sector.getSectorType().equals("Outbound") && i <= noOfSectors){
                    FormLayout formLayout = (FormLayout) mainLayout.getComponent(i-1);
    ...
    private int getObSectors(){
        ...
        if(obj != null && obj instanceof Flight){
            flight = (Flight)obj;
            noOfSectors = flight.getObNoOfSectors();
            return noOfSectors;
        }
        return 0;
    }
